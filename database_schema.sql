...
CREATE INDEX idx_buildings_gender ON buildings(gender_restriction);
...
CREATE TABLE room_applications (
    ...
    -- Google Drive link created after manager approval archives private documents.
...
CREATE INDEX idx_applications_year ON room_applications(academic_year_applied);
...
CREATE INDEX idx_assignments_application ON room_assignments(application_id);
...
CREATE TABLE utility_bills (
    ...
    -- Google Drive link created when the monthly billing report is exported.
...
CREATE INDEX idx_utility_bills_month ON utility_bills(billing_month);
...
CREATE INDEX idx_student_bills_month ON student_bills(billing_month);
...
-- ==============================================================================
-- AUTOMATIC TRIGGER: UPDATE TIMESTAMPS
-- ==============================================================================
...
DO $$
BEGIN
    RAISE NOTICE '✅ KSIT Dormitory System Database Schema Created Successfully!';
    RAISE NOTICE 'Tables created: 11';
    RAISE NOTICE 'Enum types created: 10';
    RAISE NOTICE 'Triggers created: 3';
    RAISE NOTICE 'Next step: Run the test data insert script (test_data.sql)';
END $$;
