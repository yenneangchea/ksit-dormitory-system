...
test('frontend connection fallback and backend CORS restrict credentialed previews to the KSIT application', () => {
  const api = fs.readFileSync(path.join(frontendRoot, 'lib', 'api.ts'), 'utf8');
  const app = read('app.js');
  assert.match(api, /https:\/\/ksit-dorm-api\.vercel\.app/);
