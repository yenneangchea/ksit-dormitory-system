...
import { API_BASE_URL, buildingsAPI, dashboardAPI, majorsAPI, roomAssignmentsAPI, roomsAPI, type AssignmentBoard, type DashboardAnalytics as DashboardAnalyticsData, type DashboardSummary, type PasswordResetRequest, usersAPI } from '@/lib/api';
  ...
  return (
    <PortalShell role="admin">
      <section className="min-h-[calc(100vh-156px)]">
        ...
        {activeTab === 'settings' && <SystemSettingsPanel settings={announcementManagement.settings.system_settings} majors={majors} isWorking={isWorking} onSave={saveSystemSettings} onAddMajor={() => setMajorModal('new')} onEditMajor={setMajorModal} onToggleMajor={(major) => void toggleMajor(major)} onDeleteMajor={(major) => void deleteMajor(major)} />}
      </section>
...
function SystemSettingsPanel({ settings, majors, isWorking, onSave, onAddMajor, onEditMajor, onToggleMajor, onDeleteMajor }: { settings?: SystemSettings; majors: AcademicMajor[]; isWorking: boolean; onSave: (formData: FormData) => Promise<void>; onAddMajor: () => void; onEditMajor: (major: AcademicMajor) => void; onToggleMajor: (major: AcademicMajor) => void; onDeleteMajor: (major: AcademicMajor) => void }) {
