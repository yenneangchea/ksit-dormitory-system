...
type SystemSettings = { academic_levels?: string[]; utility_rates?: { electricity_khr_per_kwh?: number; water_khr_per_m3?: number; trash_khr_per_room?: number; free_electricity_kwh?: number; free_water_m3?: number }; housing_fee?: { annual_khr?: number }; telegram?: { username?: string; webhook_configured?: boolean } };
...
function AdminDashboardContent() {
  ...
  const [majorAuditRefreshToken, setMajorAuditRefreshToken] = useState(0);
  ...
  async function load() {
    const [summaryResponse, analyticsResponse, usersResponse, buildingsResponse, announcementsResponse, resetRequestsResponse, majorsResponse] = await Promise.all([dashboardAPI.summary(), dashboardAPI.analytics(), usersAPI.list(), buildingsAPI.list(), announcementRequest<AnnouncementManagement>('/api/announcements'), usersAPI.listPasswordResetRequests('pending'), majorsAPI.listAdmin()]);
    ...
    if (majorsResponse.success && majorsResponse.data) setMajors(majorsResponse.data);
  }
  ...
  async function submitUser(formData: FormData) {
    const payload = {
      ...
      academic_level: userAcademicSelection.academic_level || undefined,
      academic_major_id: userAcademicSelection.academic_major_id || undefined,
      academic_year: userAcademicSelection.academic_year ? Number(userAcademicSelection.academic_year) : undefined,
  ...
  async function saveSystemSettings(formData: FormData) {
    ...
    await runAction(() => announcementRequest('/api/announcements/settings', { method: 'PUT', body: JSON.stringify({ system_settings: {
      academic_levels,
      utility_rates: { electricity_khr_per_kwh: Number(formData.get('electricity_rate') || 800), free_electricity_kwh: Number(formData.get('free_electricity_quota') || 50), water_khr_per_m3: Number(formData.get('water_rate') || 1500), free_water_m3: Number(formData.get('free_water_quota') || 5), trash_khr_per_room: Number(formData.get('trash_fee') || 10000) },
  ...
  async function saveMajor(formData: FormData) {
    const payload = {
      academic_level: String(formData.get('academic_level') || ''),
      name_khmer: String(formData.get('name_khmer') || ''),
      name_english: String(formData.get('name_english') || ''),
      available_year_levels: [1, 2, 3, 4].filter((year) => formData.get(`year_${year}`) === 'on'),
      is_active: formData.get('is_active') === 'on',
    };
    const response = majorModal !== 'new' && majorModal ? majorsAPI.update(majorModal.id, payload) : majorsAPI.create(payload);
    await runAction(() => response, majorModal === 'new' ? 'Academic major created.' : 'Academic major updated.');
    setMajorModal(null);
  }

  async function toggleMajor(major: AcademicMajor) {
    await runAction(() => majorsAPI.removeOrToggle(major.id, 'deactivate'), `Academic major ${major.is_active ? 'deactivated' : 'activated'}.`);
  }

  async function deleteMajor(major: AcademicMajor) {
    if (!window.confirm(`Delete “${major.name_khmer}”? Majors referenced by student profiles can be deactivated instead.`)) return;
    await runAction(() => majorsAPI.removeOrToggle(major.id, 'delete'), 'Academic major deleted.');
  }

  async function completeMajorImport(result: AcademicMajorImportResult) {
    setNotice(`Academic major import completed: ${result.created} created, ${result.updated} updated.`);
    await load();
    setMajorAuditRefreshToken((value) => value + 1);
  }
  ...
  return (
    <PortalShell role="admin">
      ...
      {modal?.type === 'user' && <Modal title={modal.user ? 'Edit user account' : 'Add new user account'} onClose={() => setModal(null)}><form action={submitUser} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><Field label="Khmer full name" name="full_name_khmer" defaultValue={modal.user?.full_name_khmer} required /><Field label="Latin full name" name="full_name_latin" defaultValue={modal.user?.full_name_latin} required /><Field label="Email address" name="email" type="email" defaultValue={modal.user?.email} required /><Field label="Phone number" name="phone" defaultValue={modal.user?.phone} required /><SelectField label="Gender" name="gender" defaultValue={modal.user?.gender || 'male'} options={[['male', 'Male'], ['female', 'Female']]} /><SelectField label="Role" name="role" defaultValue={modal.user?.role || 'student'} options={roleOptions.map((role) => [role, role[0].toUpperCase() + role.slice(1)])} /></div><div className="rounded-xl border border-[#e1e7e1] bg-[#fafcf9] p-4"><p className="mb-3 text-sm font-bold text-[#20342a]">Academic program for Student accounts</p><AcademicProgramFields value={userAcademicSelection} onChange={setUserAcademicSelection} /><p className="mt-2 text-xs text-[#68736c]">Student accounts require an active academic level, configured major, and permitted year level. Staff accounts may leave this blank.</p></div><Field label={modal.user ? 'New temporary password (optional)' : 'Temporary password'} name="password" type="password" minLength={8} required={!modal.user} hint={modal.user ? 'Leave blank to keep the current password.' : 'Minimum 8 characters.'} /><ModalActions busy={isWorking} submitLabel={modal.user ? 'Save user changes' : 'Create user'} /></form></Modal>}
      ...
      {majorModal && <Modal title={majorModal === 'new' ? 'បន្ថែមជំនាញថ្មី (Add New Major)' : 'កែប្រែជំនាញ (Edit Major)'} onClose={() => setMajorModal(null)}><form action={saveMajor} className="space-y-4"><SelectField label="Academic level" name="academic_level" defaultValue={majorModal === 'new' ? 'ផ្នែកអប់រំបច្ចេកទេស (៩+៣)' : majorModal.academic_level} options={[["ផ្នែកអប់រំបច្ចេកទេស (៩+៣)", "ផ្នែកអប់រំបច្ចេកទេស (៩+៣)"], ["កម្រិតបរិញ្ញាបត្ររង", "កម្រិតបរិញ្ញាបត្ររង"], ["កម្រិតបរិញ្ញាបត្រ", "កម្រិតបរិញ្ញាបត្រ"]]} required /><Field label="Major name (Khmer)" name="name_khmer" defaultValue={majorModal === 'new' ? '' : majorModal.name_khmer} required /><Field label="Major name (English)" name="name_english" defaultValue={majorModal === 'new' ? '' : majorModal.name_english} required /><fieldset><legend className="text-sm font-medium text-[#39473f]">Available year levels</legend><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">{[1, 2, 3, 4].map((year) => <label key={year} className="flex min-h-11 items-center gap-2 rounded-lg border border-[#dce3dc] bg-[#fafcf9] px-3 text-sm"><input type="checkbox" name={`year_${year}`} defaultChecked={majorModal === 'new' ? year === 1 : majorModal.available_year_levels.includes(year)} />Year {year}</label>)}</div></fieldset><label className="flex min-h-11 items-center gap-2 rounded-lg bg-[#f3f8f3] px-3 text-sm text-[#31513d]"><input type="checkbox" name="is_active" defaultChecked={majorModal === 'new' ? true : majorModal.is_active} />Active and available for registration</label><ModalActions busy={isWorking} submitLabel={majorModal === 'new' ? 'Create major' : 'Save major'} /></form></Modal>}
    </PortalShell>
  );
}
...
function SystemSettingsPanel({ settings, isWorking, onSave }: { settings?: SystemSettings; isWorking: boolean; onSave: (formData: FormData) => Promise<void> }) {
  const defaults = settings || { academic_levels: ['9+1', '9+2', '9+3', 'Bachelor Year 1', 'Bachelor Year 2', 'Bachelor Year 3', 'Bachelor Year 4'], utility_rates: { electricity_khr_per_kwh: 800, free_electricity_kwh: 50, water_khr_per_m3: 1500, free_water_m3: 5, trash_khr_per_room: 10000 }, housing_fee: { annual_khr: 120000 }, telegram: { username: '@KSITDorm_bot', webhook_configured: false } };
  function downloadConfig() {
    const blob = new Blob([JSON.stringify({ exported_at: new Date().toISOString(), system_settings: defaults }, null, 2)], { type: 'application/json' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'ksit-dorm-system-settings-backup.json';
    link.click();
    URL.revokeObjectURL(link.href);
  }
  return <section className="ksit-card mt-8 overflow-hidden">
    <div className="p-6"><h2 className="text-lg font-bold">⚙️ System Settings</h2><p className="mt-1 text-sm text-[#68736c]">Safe operational defaults. Academic Programs & Majors management is available only from the dedicated Academic & Majors workspace. Secret values, including Telegram bot tokens, are intentionally never shown in the browser.</p></div>
    <form action={onSave} className="grid gap-6 border-t border-[#edf0ed] p-6 xl:grid-cols-2"><div className="space-y-4"><label className="block text-sm font-medium text-[#39473f]">Legacy academic settings<textarea name="academic_levels" defaultValue={(defaults.academic_levels || []).join('\n')} className="mt-1.5 min-h-28 w-full rounded-xl border border-[#dce3dc] bg-white p-3 text-sm outline-none focus:border-[#5f9b6f]" /></label><div className="rounded-xl border border-[#e1e7e1] bg-[#fafcf9] p-4"><h3 className="text-sm font-bold text-[#20342a]">Utility rates & free quotas</h3><p className="mt-1 text-xs leading-5 text-[#68736c]">Each room receives the configured free electricity and water quota before the remaining cost is split among active residents.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="Electricity rate (៛/kWh)" name="electricity_rate" type="number" min="0" defaultValue={String(defaults.utility_rates?.electricity_khr_per_kwh ?? 800)} required /><Field label="Free electricity quota (kWh/room)" name="free_electricity_quota" type="number" min="0" step="0.01" defaultValue={String(defaults.utility_rates?.free_electricity_kwh ?? 50)} required /><Field label="Water rate (៛/m³)" name="water_rate" type="number" min="0" defaultValue={String(defaults.utility_rates?.water_khr_per_m3 ?? 1500)} required /><Field label="Free water quota (m³/room)" name="free_water_quota" type="number" min="0" step="0.01" defaultValue={String(defaults.utility_rates?.free_water_m3 ?? 5)} required /><Field label="Trash fee (៛/room)" name="trash_fee" type="number" min="0" defaultValue={String(defaults.utility_rates?.trash_khr_per_room ?? 10000)} required /><Field label="Annual housing fee (៛)" name="housing_fee" type="number" min="0" defaultValue={String(defaults.housing_fee?.annual_khr ?? 120000)} required /></div></div></div><div className="space-y-4 rounded-xl border border-[#e1e7e1] bg-[#fafcf9] p-4"><Field label="Telegram bot username" name="telegram_username" defaultValue={defaults.telegram?.username || '@KSITDorm_bot'} required /><p className="rounded-lg bg-white p-3 text-sm text-[#405349]">Webhook status: <b className={defaults.telegram?.webhook_configured ? 'text-[#0b6937]' : 'text-[#a66b10]'}>{defaults.telegram?.webhook_configured ? 'Configured on the server' : 'Needs server configuration'}</b></p><button type="button" onClick={downloadConfig} className="w-full rounded-lg border border-[#dce3dc] bg-white px-3 py-2 text-xs font-bold text-[#31513d] hover:bg-[#f4f8f4]">Download configuration backup</button><p className="text-xs leading-5 text-[#68736c]">For privacy and data safety, full database backup and restore remain an administrator-operated Supabase/pg_dump procedure and are not executed from this browser.</p></div><div className="xl:col-span-2"><ModalActions busy={isWorking} submitLabel="Save system settings" /></div></form>
  </section>;
}
