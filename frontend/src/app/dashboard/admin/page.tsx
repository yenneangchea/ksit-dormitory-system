...
function AdminDashboardContent() {
  ...
  async function load() {
    const [summaryResponse, analyticsResponse, usersResponse, buildingsResponse, announcementsResponse] = await Promise.all([dashboardAPI.summary(), dashboardAPI.analytics(), usersAPI.list(), buildingsAPI.list(), announcementRequest<AnnouncementManagement>('/api/announcements')]);
    ...
    <PortalShell role="admin">
      <section className="min-h-[calc(100vh-156px)]">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">{activeTab === 'users' && <ActionButton icon={<Plus />} onClick={() => setModal({ type: 'user' })}>Add new user</ActionButton>}{activeTab === 'residence' && <><ActionButton icon={<Building2 />} tone="secondary" onClick={() => setModal({ type: 'building' })}>Add building</ActionButton><ActionButton icon={<QrCode />} tone="secondary" onClick={() => setModal({ type: 'room' })}>Add room</ActionButton></>}{activeTab === 'cms' && <Link href="/dashboard/admin/homepage-editor" className="inline-flex items-center gap-2 rounded-lg border border-[#dce3dc] bg-white px-3 py-2 text-xs font-bold text-[#31513d] hover:bg-[#f4f8f4]">🎨 Edit Homepage</Link>}</div>
        </div>
