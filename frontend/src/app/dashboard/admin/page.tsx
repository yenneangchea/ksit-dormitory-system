...
function SystemSettingsPanel({ settings, majors, isWorking, onSave, onAddMajor, onEditMajor, onToggleMajor, onDeleteMajor, auditRefreshToken, onImportComplete }: { settings?: SystemSettings; majors: AcademicMajor[]; isWorking: boolean; onSave: (formData: FormData) => Promise<void>; onAddMajor: () => void; onEditMajor: (major: AcademicMajor) => void; onToggleMajor: (major: AcademicMajor) => void; onDeleteMajor: (major: AcademicMajor) => void; auditRefreshToken: number; onImportComplete: (result: AcademicMajorImportResult) => Promise<void> }) {
  ...
  return <section className="ksit-card mt-8 overflow-hidden">
    <div className="p-6"><h2 className="text-lg font-bold">⚙️ System Settings</h2><p className="mt-1 text-sm text-[#68736c]">Academic structures and safe operational defaults. Secret values, including Telegram bot tokens, are intentionally never shown in the browser.</p></div>
    <AcademicMajorsManager majors={majors} isWorking={isWorking} onAddMajor={onAddMajor} onEditMajor={onEditMajor} onToggleMajor={onToggleMajor} onDeleteMajor={onDeleteMajor} auditRefreshToken={auditRefreshToken} onImportComplete={onImportComplete} />
