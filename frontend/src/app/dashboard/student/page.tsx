...
import { StudentApplicationWizard } from '@/components/student-application-wizard';
import { UtilitySubsidyBreakdown } from '@/components/utility-subsidy-breakdown';
...
function StudentDashboardContent() {
  ...
  useEffect(() => { if (!isAuthorized) return; const timer = window.setTimeout(() => void load(), 0); return () => window.clearTimeout(timer); }, [isAuthorized, load]);
  async function submitMaintenance(formData: FormData) { setWorking(true); const response = await maintenanceAPI.create({ magic_qr_code: String(formData.get('magic_qr_code') || ''), category: String(formData.get('category') || 'other'), urgency: String(formData.get('urgency') || 'medium'), title: String(formData.get('title') || ''), description: String(formData.get('description') || '') }); setWorking(false); setNotice(response.success ? 'Maintenance ticket submitted to the residence team.' : response.error?.message || 'Unable to submit your maintenance request.'); if (response.success) await load(); }
  async function confirmBillPayment(bill: StudentBill) { const transaction_ref = window.prompt('Enter the KHQR / bank transaction reference for this payment:'); if (!transaction_ref?.trim()) return; setWorking(true); const response = await billingAPI.markPaid(bill.id, { transaction_ref: transaction_ref.trim(), payment_method: 'khqr' }); setWorking(false); setNotice(response.success ? 'Payment reference recorded. The residence office can reconcile the transaction.' : response.error?.message || 'Unable to record the payment reference.'); if (response.success) await load(); }
...
function Bills({ bills, working, onMarkPaid }: { bills: StudentBill[]; working: boolean; onMarkPaid: (bill: StudentBill) => Promise<void> }) { return <section className="ksit-card overflow-hidden"><div className="p-6"><h2 className="text-lg font-bold">My dynamic KHQR bills</h2><p className="mt-1 text-sm text-[#68736c]">Your invoice shows the electricity and water subsidy applied to your room before the exact per-student KHQR amount is calculated.</p></div>{bills.length === 0 ? <p className="border-t border-[#edf0ed] px-6 py-7 text-sm text-[#68736c]">No utility bills are due at this time.</p> : <div className="border-t border-[#edf0ed]">{bills.map((bill) => <div className="border-b border-[#edf0ed] px-6 py-5" key={bill.id}><div className="grid gap-4 md:grid-cols-[1fr_auto_auto] md:items-center"><div><p className="font-semibold">Utility bill · {bill.billing_month}</p><p className="mt-1 text-xs text-[#68736c]">Room {bill.room_id.slice(0, 8)} · Payment reference generated for this invoice.</p><code className="mt-3 block max-w-full overflow-x-auto rounded-lg bg-[#f1f6f1] px-3 py-2 text-xs text-[#28533a]">{bill.khqr_string}</code></div><div><p className="text-xs text-[#68736c]">Exact KHQR amount due</p><p className="mt-1 text-xl font-extrabold text-[#0b5c2c]">៛ {new Intl.NumberFormat('en-US', { maximumFractionDigits: 0 }).format(Number(bill.amount_khr))}</p></div><div className="flex flex-col items-start gap-2"><span className={`w-fit rounded-full px-2.5 py-1 text-xs font-semibold capitalize ${bill.bill_status === 'paid' ? 'bg-[#eaf6ec] text-[#1a6a37]' : 'bg-[#f5f0e7] text-[#806525]'}`}>{bill.bill_status}</span>{bill.bill_status !== 'paid' && <button disabled={working} onClick={() => void onMarkPaid(bill)} className="rounded-lg bg-[#0b5c2c] px-3 py-1.5 text-xs font-bold text-white disabled:opacity-50">Confirm KHQR payment</button>}</div></div><UtilitySubsidyBreakdown bill={bill.utility_bills} /></div>)}</div>}</section>; }
