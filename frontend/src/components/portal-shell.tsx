  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <aside className={`flex h-full flex-col border-r border-[#dfe5df] bg-white ${mobile ? 'w-72 shadow-2xl' : collapsed ? 'w-[78px]' : 'w-[276px]'} transition-[width] duration-200`}>
      <div className={`flex h-[76px] items-center border-b border-[#edf0ed] ${collapsed && !mobile ? 'justify-center px-2' : 'justify-between px-4'}`}>
        ...
        {mobile ? <button type="button" onClick={() => setMobileOpen(false)} className="flex size-11 items-center justify-center rounded-lg text-[#31513d] hover:bg-[#edf5ee]" aria-label="Close menu"><X className="size-5" /></button> : <button type="button" onClick={() => setCollapsed((value) => !value)} className="rounded-lg p-2 text-[#31513d] hover:bg-[#edf5ee]" aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'} title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}>{collapsed ? <ChevronRight className="size-5" /> : <ChevronLeft className="size-5" />}</button>}
      </div>
