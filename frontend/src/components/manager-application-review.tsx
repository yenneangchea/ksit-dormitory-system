...
export function ManagerApplicationReview() {
  ...
  const load = useCallback(async () => {
    ...
    if (!response.success || !response.data) { setError(response.error?.message || 'Unable to load applications.'); return; }
  ...
  const totals = useMemo(() => ({ review: applications.filter((item) => item.status === 'under_review').length, approved: applications.filter((item) => ['approved', 'assigned'].includes(item.status)).length }), [applications]);
  ...
  async function decide(action: 'approve' | 'request_correction' | 'reject') {
    ...
    setBusy(false);
    if (!response.success || !response.data) { setError(response.error?.message || 'Unable to record the manager decision.'); return; }
