...
function initialSiblings(application: RoomApplication | null): ApplicationSibling[] {
  const stored = application?.draft_data?.siblings_json || application?.form_data_json?.siblings_json;
...
function initialEducation(application: RoomApplication | null): ApplicationEducation[] {
  const stored = application?.draft_data?.education_history_json || application?.form_data_json?.education_history_json;
...
function initialContacts(application: RoomApplication | null): ApplicationEmergencyContact[] {
  const stored = application?.draft_data?.emergency_contacts_json || application?.form_data_json?.emergency_contacts_json;
  const profile = firstProfile(application)?.emergency_contacts_json;
  const value = Array.isArray(stored) ? stored : Array.isArray(profile) ? profile : [];
  return value.length ? [value[0] as ApplicationEmergencyContact] : [{ name: '', relation: '', phone: '', address: '' }];
}

function initialSection(application: RoomApplication | null) {
  const candidate = Number(application?.step_progress || application?.draft_data?.step_progress || 1);
  return Number.isInteger(candidate) ? Math.min(4, Math.max(1, candidate)) : 1;
}
...
export function StudentApplicationWizard({ applications, onUpdated }: { applications: RoomApplication[]; onUpdated: () => Promise<void> }) {
  ...
  const [section, setSection] = useState(initialSection(applications[0] || null));
  ...
  const preservedDraftSection = useRef<number | null>(null);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      const savedSection = preservedDraftSection.current;
      ...
      setSection(savedSection || initialSection(latest));
      preservedDraftSection.current = null;
  ...
  async function saveDraft(nextStage = 1, sectionProgress = section) {
    setBusy(true);
    const response = await applicationsAPI.saveDraft({ academic_year_applied: String(form.academic_year_applied || '2025-2026'), step_progress: sectionProgress, form_data: { ...form, step_progress: sectionProgress, siblings_json: siblings, education_history_json: education, emergency_contacts_json: contacts } });
    setBusy(false);
    ...
    preservedDraftSection.current = sectionProgress;
    preserveCurrentForm();
  ...
  async function upload(type: 'student_photo' | 'national_id' | 'family_book') {
    ...
    if (file.size > 5 * 1024 * 1024) { setNotice('ឯកសារនីមួយៗត្រូវមានទំហំមិនលើស 5 MB។'); return; }
    setUploadProgress(0);
    setBusy(true);
  ...
  async function uploadSigned() {
    ...
    if (files.signed_application.size > 5 * 1024 * 1024) { setNotice('ឯកសារនីមួយៗត្រូវមានទំហំមិនលើស 5 MB។'); return; }
    setUploadProgress(0);
    setBusy(true);
  ...
  return <section className="space-y-6">
    ...
    {stage === 1 && <section className="ksit-card p-5 sm:p-6"><div className="flex flex-wrap gap-2 border-b border-[#edf0ed] pb-4">{formSections.map((item) => <button key={item.id} type="button" onClick={() => setSection(item.id)} className={`min-h-10 rounded-lg px-3 text-xs font-bold ${section === item.id ? 'bg-[#0b5c2c] text-white' : 'bg-[#f3f6f3] text-[#526058] hover:bg-[#e7efe8]'}`}>{item.title}</button>)}</div><div className="pt-5">{section === 1 && <SectionOne form={form} update={updateForm} />}{section === 2 && <SectionTwo form={form} update={updateForm} siblings={siblings} setSiblings={setSiblings} education={education} setEducation={setEducation} />}{section === 3 && <SectionThree form={form} update={updateForm} />}{section === 4 && <SectionFour form={form} update={updateForm} contacts={contacts} setContacts={setContacts} />}</div><div className="mt-6 flex flex-wrap justify-between gap-3 border-t border-[#edf0ed] pt-5"><button type="button" disabled={section === 1} onClick={() => setSection((value) => Math.max(1, value - 1))} className="inline-flex min-h-11 items-center gap-1 rounded-lg border border-[#dce3dc] px-4 text-sm font-bold text-[#526058] disabled:opacity-50"><ChevronLeft className="size-4" />ផ្នែកមុន</button><button type="button" disabled={busy} onClick={() => void saveDraft(1, section)} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-[#0b5c2c] px-4 text-sm font-bold text-[#0b5c2c] disabled:opacity-60">{busy && <Loader2 className="size-4 animate-spin" />}រក្សាទុកពង្រាង</button>{section < 4 ? <button type="button" onClick={() => setSection((value) => Math.min(4, value + 1))} className="inline-flex min-h-11 items-center gap-1 rounded-lg bg-[#0b5c2c] px-4 text-sm font-bold text-white">ផ្នែកបន្ទាប់<ChevronRight className="size-4" /></button> : <button type="button" disabled={busy} onClick={() => void saveDraft(2, 4)} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-[#0b5c2c] px-4 text-sm font-bold text-white disabled:opacity-60">{busy && <Loader2 className="size-4 animate-spin" />}រក្សាទុក និងបន្តទៅឯកសារ<ChevronRight className="size-4" /></button>}</div></section>}
    {stage === 2 && <UploadStage application={application} files={files} setFiles={setFiles} busy={busy} progress={uploadProgress} onUpload={upload} onBack={() => setStage(1)} onContinue={() => setStage(3)} />}
...
function SignedStage({ file, setFile, busy, progress, onSubmit, onBack }: { file: File | null; setFile: (file: File | null) => void; busy: boolean; progress: number | null; onSubmit: () => Promise<void>; onBack: () => void }) { return <section className="ksit-card p-5 sm:p-6"><SectionHeading title="ផ្ទុកឯកសារដែលបានចុះហត្ថលេខា និងផ្ដិតមេដៃ" description="រួមបញ្ចូលហត្ថលេខា/ស្នាមមេដៃស្តាំរបស់សិស្សលើទំព័រ ១, ២, ៣ និងឪពុកម្តាយ/អាណាព្យាបាលលើទំព័រ ៣, ៤។" /><label className="mt-5 flex min-h-44 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-[#b7cfba] bg-[#f8fcf8] p-6 text-center"><UploadCloud className="size-9 text-[#0b5c2c]" /><span className="mt-3 text-sm font-bold">ជ្រើស PDF ឬរូបភាពដែលបានស្កេន</span><span className="mt-1 text-xs text-[#68736c]">PDF, PNG ឬ JPG · អតិបរមា 5 MB</span><input type="file" className="sr-only" accept="application/pdf,image/png,image/jpeg" onChange={(event) => setFile(event.target.files?.[0] || null)} /></label><p className="mt-3 text-center text-xs text-[#68736c]">{file?.name || 'មិនទាន់ជ្រើសឯកសារ'}</p>{busy && progress !== null && <div className="mx-auto mt-3 max-w-md" aria-live="polite"><div className="h-1.5 overflow-hidden rounded bg-[#dcebdd]"><div className="h-full bg-[#0b5c2c] transition-all" style={{ width: `${progress}%` }} /></div><p className="mt-1 text-center text-[11px] font-semibold text-[#287243]">Uploading {progress}%</p></div>}<div className="mt-6 flex flex-wrap justify-between gap-3 border-t border-[#edf0ed] pt-5"><button type="button" onClick={onBack} className="inline-flex min-h-11 items-center gap-1 rounded-lg border border-[#dce3dc] px-4 text-sm font-bold text-[#526058]"><ChevronLeft className="size-4" />ត្រឡប់ក្រោយ</button><button type="button" disabled={busy || !file} onClick={() => void onSubmit()} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-[#0b5c2c] px-4 text-sm font-bold text-white disabled:opacity-50">{busy && <Loader2 className="size-4 animate-spin" />}បញ្ជូនឱ្យអ្នកគ្រប់គ្រងពិនិត្យ<ChevronRight className="size-4" /></button></div></section>; }
...
function DynamicContacts({ rows, setRows }: { rows: ApplicationEmergencyContact[]; setRows: React.Dispatch<React.SetStateAction<ApplicationEmergencyContact[]>> }) { const update = (index: number, key: keyof ApplicationEmergencyContact, value: string) => setRows((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, [key]: value } : row)); return <div>{rows.slice(0, 1).map((row, index) => <div key={index} className="grid gap-3 rounded-xl border border-[#e1e7e1] p-3 md:grid-cols-2"><Text label="ឈ្មោះ" value={row.name} onChange={(value) => update(index, 'name', value)} /><Text label="ត្រូវជា" value={row.relation || ''} onChange={(value) => update(index, 'relation', value)} /><Text label="ទីលំនៅ/កន្លែងធ្វើការ" value={row.address || ''} onChange={(value) => update(index, 'address', value)} /><Text label="លេខទូរស័ព្ទ" value={row.phone || ''} onChange={(value) => update(index, 'phone', value)} /></div>)}</div>; }
